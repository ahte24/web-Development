...
async function getSongs(folder) {
	...
	let a = await fetch(
		`http://192.168.1.6:5000/Music%20Player%20X%20Spotify/${folder}/`
	);
...
const playMusic = (track, pause = false) => {
	...
	currentSong.src =
		`http://192.168.1.6:5000/Music%20Player%20X%20Spotify/${currFolder}/` +
		track;
...
async function displayAlbums() {
	let a = await fetch(
		`http://192.168.1.6:5000/Music%20Player%20X%20Spotify/musics/`
	);
	let response = await a.text();
	let div = document.createElement("div");
	div.innerHTML = response;
	...
	for (let index = 0; index < array.length; index++) {
		...
		if (e.href.includes("/musics/") && !e.href.includes(".htaccess")) {
			...
			let a = await fetch(`http://192.168.1.6:5000/Music%20Player%20X%20Spotify/musics/${folder}/info.json`);
			...
			<img src="http://192.168.1.6:5000/Music%20Player%20X%20Spotify/musics/${folder}/cover.jpg" alt="" />
